...
@endphp
    ...
    <section class="relative min-h-[90vh] flex items-end overflow-hidden bg-slate-900 -mt-16">
        <div class="absolute inset-0" style="background-image: url('{{ $heroImg }}'); background-size: cover; background-position: center 40%;"></div>
        ...
        <div class="relative z-10 max-w-7xl mx-auto px-6 pb-24 pt-40 w-full">
            <div class="max-w-3xl">
                <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-amber-400/40 bg-amber-400/10 mb-6">
                    <div class="w-1.5 h-1.5 rounded-full bg-amber-400"></div>
                    <span class="text-xs font-mono text-amber-400 tracking-widest uppercase">Plataforma Oficial · 2026</span>
                </div>
                ...
                <p class="text-lg text-slate-300 max-w-xl mb-10 leading-relaxed">
                    Visualize, analise e simule trajetos sobre a malha ferroviária nacional. Dados operacionais em tempo real de todas as concessões ativas.
                </p>
                <div class="flex flex-wrap gap-4">
                    <a href="{{ route('simulador') }}" class="inline-flex items-center gap-3 px-7 py-3.5 rounded-lg bg-amber-400 text-slate-900 font-bold text-sm hover:bg-amber-300 transition-colors">
                        Explorar Mapa →
                    </a>
                </div>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-6 py-20">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach($concessionaires as $c)
                    <div class="group relative p-5 rounded-xl border border-slate-200 bg-white hover:border-slate-300 hover:shadow-md transition-all overflow-hidden cursor-default">
    ...
    <section class="bg-white border-b border-slate-200">
        <div class="max-w-7xl mx-auto px-6 py-20">
            ...
            <h2 class="text-3xl lg:text-4xl font-bold text-slate-900 tracking-tight mb-12">Principais Produtos Transportados</h2>
                ...
                @foreach($cargas as $item)
                    <div class="group rounded-xl overflow-hidden border border-slate-200 hover:shadow-lg transition-all">
                        <div class="relative h-48 overflow-hidden bg-slate-100">
                            <img src="{{ $item['img'] }}" alt="{{ $item['tag'] }}" class="w-full h-full object-cover opacity-80 group-hover:opacity-95 group-hover:scale-105 transition-all duration-500" />
                            <div class="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent"></div>
                            <div class="absolute top-3 left-3 px-2.5 py-1 rounded text-xs font-semibold text-white" style="background: {{ $item['color'] }}cc">{{ $item['tag'] }}</div>
                            <div class="absolute bottom-3 right-3 font-bold text-3xl text-white font-mono drop-shadow">{{ $item['pct'] }}</div>
                        </div>
    ...
    <section class="bg-[#f8f7f4] border-b border-slate-200">
        <div class="max-w-7xl mx-auto px-6 py-20">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                @foreach($news as $n)
                    <article class="group flex flex-col border border-slate-200 rounded-xl overflow-hidden hover:shadow-lg hover:border-slate-300 transition-all bg-white cursor-pointer">
                        <div class="relative h-40 bg-slate-100 overflow-hidden">
                            <img src="{{ $n['img'] }}" alt="{{ $n['title'] }}" class="w-full h-full object-cover group-hover:scale-105 transition-all duration-500" />
                        ...
                        <div class="flex-1 p-5 flex flex-col gap-3">
                            ...
                            <h3 class="font-semibold text-slate-800 leading-snug text-sm group-hover:text-slate-900 transition-colors">{{ $n['title'] }}</h3>
    ...
    <!-- MAPA INTERATIVO -->
    <section id="simulador" class="relative w-full h-[70vh] min-h-[600px] bg-slate-200 border-t border-slate-300 z-10 mt-12">
        <!-- Div onde o Leaflet vai renderizar -->
        <div id="map" class="absolute inset-0 w-full h-full z-10"></div>
        
        <!-- Painel de Controle -->
        <div class="absolute top-6 left-6 z-[400] bg-white/95 backdrop-blur p-5 rounded-xl shadow-xl border border-slate-200 w-80 md:w-96 pointer-events-auto">
            <div class="inline-flex items-center gap-2 px-2 py-1 rounded bg-amber-100 text-amber-700 text-xs font-bold uppercase tracking-widest mb-3">
                Simulador Ativo
            </div>
            ...
            <div class="p-4 border-2 border-dashed border-slate-200 rounded-lg text-center text-sm text-slate-400 font-mono">
                <!-- Filtro de Concessionárias -->
            <div class="mt-4">
                <label for="filtro-concessionaria" class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">
                    Visualizar Malha
                </label>
                <select id="filtro-concessionaria" class="w-full bg-slate-50 border border-slate-200 text-slate-700 text-sm font-medium rounded-lg focus:ring-amber-500 focus:border-amber-500 block p-3 outline-none transition-all cursor-pointer shadow-sm">
