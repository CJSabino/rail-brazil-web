...
<style>
    body { font-family: 'Inter', sans-serif; background-color: #f8f7f4; color: #0f172a; }
    #map-container { position: relative; height: 750px; border-radius: 1rem; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); border: 1px solid #e2e8f0; }
    #map { height: 100%; width: 100%; z-index: 10; cursor: crosshair; }
    .timeline-container { position: relative; }
    .timeline-container::before { content: ''; position: absolute; left: 15px; top: 15px; bottom: 15px; width: 2px; background: #e2e8f0; z-index: 1; }
    .timeline-item { position: relative; padding-left: 2.5rem; }
    .timeline-item::before { content: ''; position: absolute; left: 11px; top: 50%; transform: translateY(-50%); width: 10px; height: 10px; background: #f59e0b; border-radius: 50%; border: 2px solid #fff; box-shadow: 0 0 0 3px #fef3c7; z-index: 2; transition: all 0.3s ease; }
    .timeline-item.endpoint::before { width: 14px; height: 14px; left: 9px; background: #10b981; box-shadow: 0 0 0 3px #d1fae5; }
    .loader { border-top-color: #f59e0b; animation: spinner 1s linear infinite; }
    @keyframes spinner { to { transform: rotate(360deg); } }
    .custom-tooltip { background: rgba(255, 255, 255, 0.95); color: #0f172a; border: 1px solid #e2e8f0; border-radius: 8px; padding: 6px 12px; font-weight: 600; font-size: 13px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); backdrop-filter: blur(4px); }
...
<div class="max-w-[1600px] mx-auto px-4 mt-6 mb-12">
    <div class="mb-4">
        ...
        <p class="text-sm text-slate-500">Selecione origem e destino para calcular a rota, tempo e custos operacionais.</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="col-span-1 lg:col-span-8">
            <div id="map-container">
                <!-- Loader -->
                <div id="map-loader" class="hidden absolute inset-0 bg-white/70 z-[2000] flex flex-col items-center justify-center backdrop-blur-sm transition-opacity duration-300">
                    <div class="loader rounded-full border-4 border-t-4 border-slate-200 h-16 w-16 mb-4 shadow-lg"></div>
                ...
                <div class="absolute top-0 left-0 right-0 z-[1000] p-4 bg-gradient-to-b from-black/20 to-transparent pointer-events-none">
                    <div class="inline-flex items-center gap-3 px-4 py-1.5 rounded-full bg-white border border-slate-200 shadow-sm pointer-events-none">
                        <div class="flex items-center gap-1.5">
                            <div class="w-1.5 h-1.5 rounded-full bg-emerald-500"></div>
                            <span class="font-mono text-[10px] uppercase text-slate-500 tracking-wider">Malha Nacional Ativa</span>
                        </div>
                    </div>
                </div>

                <!-- Overlay Bottom Center (Instrução Dinâmica) -->
                <div id="instrucao-simulador" class="absolute bottom-8 left-1/2 -translate-x-1/2 z-[1000] px-5 py-2.5 rounded-full bg-white border border-slate-200 shadow-md transition-all duration-300 text-center">
                ...
                <div class="absolute bottom-6 right-6 z-[1000] rounded-xl bg-white/95 backdrop-blur-sm border border-slate-200 p-4 shadow-lg pointer-events-auto">
                    <div class="text-[10px] font-mono text-slate-400 uppercase tracking-widest mb-3">Concessionárias</div>
                    <div class="space-y-2">
                        <div class="flex items-center gap-2"><div class="w-6 h-0.5" style="background:#dc2626; box-shadow:0 0 4px #dc262666"></div><span class="text-xs text-slate-600">VALE (EFC/EFVM)</span></div>
                        <div class="flex items-center gap-2"><div class="w-6 h-0.5" style="background:#f59e0b; box-shadow:0 0 4px #f59e0b66"></div><span class="text-xs text-slate-600">Rumo Logística</span></div>
                        <div class="flex items-center gap-2"><div class="w-6 h-0.5" style="background:#10b981; box-shadow:0 0 4px #10b98166"></div><span class="text-xs text-slate-600">MRS</span></div>
                        <div class="flex items-center gap-2"><div class="w-6 h-0.5" style="background:#8b5cf6; box-shadow:0 0 4px #8b5cf666"></div><span class="text-xs text-slate-600">VLI / FCA</span></div>
                        <div class="flex items-center gap-2"><div class="w-6 h-0.5" style="background:#2563eb; box-shadow:0 0 4px #2563eb66"></div><span class="text-xs text-slate-600">Transnordestina</span></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-span-1 lg:col-span-4 flex flex-col gap-4">
            ...
            <div class="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm">
                ...
                <div class="mb-4">
                    ...
                    <div class="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm font-semibold text-slate-800 truncate" id="ui-origem-nome">
                ...
                <div>
                    ...
                    <div class="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm font-semibold text-slate-800 truncate" id="ui-destino-nome">
                        Aguardando seleção no mapa...
                    </div>
            ...
            <div class="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm relative overflow-hidden">
                <h3 class="text-xs font-mono text-amber-600 tracking-widest uppercase mb-4 border-b border-slate-100 pb-2">Parâmetros da Carga</h3>
                
                <div class="mb-4">
                    <label class="text-slate-500 font-semibold text-xs mb-1.5 block">Mercadoria</label>
                    <select id="input-tipo-carga" onchange="seRotaProntaRecalcular()" class="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 text-slate-800 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none cursor-pointer transition-all">
                ...
                <div class="mb-6">
                    ...
                    <div class="flex items-center bg-slate-50 border border-slate-200 rounded-lg px-4 py-2 focus-within:ring-2 focus-within:ring-amber-500 transition-all">
                        <input type="number" id="input-carga" value="5000" class="bg-transparent w-full text-xl font-bold text-slate-800 outline-none" onchange="seRotaProntaRecalcular()">
                ...
                <div class="grid grid-cols-2 gap-4 border-t border-slate-100 pt-4">
                    <div>
                        ...
                        <p class="text-xl font-black text-slate-800"><span id="res-km">0.0</span> <span class="text-xs font-medium text-slate-500">km</span></p>
                    </div>
                    <div>
            ...
            <!-- Manifesto -->
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex-1 flex flex-col min-h-[300px] max-h-[380px]">
                <h3 class="text-xs font-mono text-amber-600 tracking-widest uppercase mb-4 border-b border-slate-100 pb-2">
                    Manifesto de Percurso
                </h3>

                <div id="alerta-transbordo" class="hidden mb-4 p-3 bg-red-50 border border-red-100 rounded-lg flex items-start gap-3">
                    <span class="text-lg leading-none">⚠️</span>
                    <p class="text-xs text-red-700 leading-relaxed">Transbordo detectado: <b id="num-transbordos"></b> quebra(s) de bitola. Custos extras aplicados.</p>
                </div>

                <div id="empty-state" class="flex-1 flex flex-col items-center justify-center text-center py-6">
                    <div class="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center border border-slate-100 mb-3 text-slate-300">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
                    </div>
                    <p class="text-xs font-medium text-slate-400 px-4">Aguardando seleção de rota no mapa.</p>
                </div>

                <div class="overflow-y-auto flex-1 pr-2 timeline-container hidden custom-scrollbar" id="timeline-wrapper">
                    <ul id="lista-percurso" class="space-y-3 py-1"></ul>
                </div>

                <button onclick="resetarSimulador()" id="btn-reset" class="mt-4 w-full py-3 bg-slate-900 text-white text-sm font-bold rounded-lg hover:bg-slate-800 transition-colors hidden shadow-sm">
                    Nova Simulação
                </button>
            </div>
        </div>
    ...
    <div id="dashboard-custos" class="grid grid-cols-1 md:grid-cols-4 gap-4 opacity-40 grayscale transition-all duration-700 pointer-events-none mt-6">
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
            ...
            <p class="text-xl font-black text-slate-800 mt-2">
                <span class="text-emerald-500 font-bold">↓</span> <span id="res-co2">0</span> <span class="text-xs font-medium text-slate-500">kg CO₂</span>
            </p>
        </div>
    </div>
</div>
